...
static int zse_tools_textEditor_render(WINDOW *win, zse_Vi2 start, zse_Vi2 end, FileData_t *f, int At_line, ZSE_Cursor_t *c)
{
	int ch;
...
int zse_tools_textEditor()
{
	...
	while(TRUE)
	{
		...
		clear();
		zse_tools_textEditor_render(stdscr, (zse_Vi2) {0, 0}, (zse_Vi2){getmaxx(stdscr), getmaxy(stdscr)}, &f, render_AtLine, &cursor);
