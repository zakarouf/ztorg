...
static int _zse_sprites_edtior(SPRITES_t *spr)
{
    ...
    while(TRUE)
    {
        ...
        switch(getch()){
            ...
            case 'p':
                brush.ink &= 0xFF00;
                brush.ink |= spr->plot[zse_xyz3Dto1D(brush.x, brush.y, brush.z, spr->X, spr->Y)]&0x00FF;
                break;
            ...
            case 'l':
                brush.ink &= 0x00FF;
                brush.ink |= brush.ink |= spr->plot[zse_xyz3Dto1D(brush.x, brush.y, brush.z, spr->X, spr->Y)]&0xFF00;
                break;
            case '-':
                brush_colo --;
