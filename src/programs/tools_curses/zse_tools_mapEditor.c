...
static int _zse_editworld_st(ST_WORLD_t* map, TILESET_t* t, char name[])
{
	...
	zse_rtC_map_show2dworld(stdscr, t->tile ,map->chunk, map->Xsize, map->Ysize, map->Zsize, brush.pos.x, brush.pos.y, brush.pos.z, 3, 1, TRUE);
	scroll(log);
	...
	while(TRUE)
	{
		...
		switch(key)
		{
			...
			case 'w':if(brush.pos.y -1 >= 0)brush.pos.y--;
				break;
			case 's':if(brush.pos.y +1 < map->Ysize)brush.pos.y++;
				break;
			case 'a':if(brush.pos.x -1 >= 0)brush.pos.x--;
				break;
			case 'd':if(brush.pos.x +1 < map->Xsize)brush.pos.x++;
				break;
			case 'z':
				if(brush.pos.z +1 < map->Zsize){brush.pos.z++;}
				break;
			case 'x':if(brush.pos.z -1 >= 0){brush.pos.z--;}
				break;
		...
		if(brush.toggle)
		{
			zse_map_draw_circle (map, brush.pos.x, brush.pos.y, brush.pos.z , brush.size , brush.ink);
		}
		...
		// Draw Routine
		zse_rtC_map_show2dworld(stdscr, t->tile ,map->chunk, map->Xsize, map->Ysize, map->Zsize, brush.pos.x, brush.pos.y, brush.pos.z, 3, 6, TRUE);

		mvwprintw(status, 0, 0,"Brush[%3s] -> ID.%3d {%s} :: %d\n", Toggle[brush.toggle], brush.ink , "NONE", brush.size);
		mvwprintw(status, 1, 0, "X - %3d || Y - %3d || Z - %3d", brush.pos.x, brush.pos.y, brush.pos.z);
